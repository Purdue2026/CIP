...
RESULT_PATH = 'EDA/results/eda_correlation_results.txt'
...
# 1) Raw dataset1 (2021년만 사용, dataset2/2022년은 미사용)
FILES = {
    'dataset1': 'data/raw/kwater_recipe06_dataset1.xlsx',
}
raw_dfs = {}
for label, path in FILES.items():
    df = pd.read_excel(path, sheet_name='RO1')
    df['Date'] = pd.to_datetime(df['Date'])
    for col in COLUMNS_RAW:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    raw_dfs[label] = df
    corr_block(df, COLUMNS_RAW, label)

# TDS vs 전기전도도 다중공선성 확인 (논문 4.1절 언급: r=1.0)
log('\n--- TDS vs 전기전도도 다중공선성 확인 (논문 4.1절 결과 검증) ---')
for label, df in raw_dfs.items():
    sub = df[['원수 TDS', '원수 전기전도도']].dropna()
    r = sub['원수 TDS'].corr(sub['원수 전기전도도'])
    log(f'  {label}: r = {r:.6f} (n={len(sub)})')

# 2) dataset_stage1.csv (이상치/결측치 처리 전, 사이클+기상만 붙인 1차본, 2021년 전체)
log('\n\n' + '=' * 70)
...
# 3) 비플럭스 중심 - 시차(lag) 상관 분석 (기상/수질 변수가 비플럭스에 미치는 지연 영향)
log('\n\n' + '=' * 70)
...
# lag-correlation curve plot (rain, temp 는 논문 핵심 변수이므로 시각화)
...
# 4) 핵심 산점도 (비플럭스, 운영차압 vs 원수 수질/기상)
log('\n\n' + '=' * 70)
